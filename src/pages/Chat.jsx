...
import { Search, MoreVertical, Paperclip, Image as ImageIcon, Send, User, Plus } from 'lucide-react';
import './Chat.css';
      ...
      <div className="chat-layout container" style={{ paddingTop: '2rem', paddingBottom: '2rem' }}>
         <div className="chat-container card" style={{ display: 'flex', height: 'calc(100vh - 12rem)', overflow: 'hidden', padding: 0 }}>
            ...
            <main className="chat-main" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
               ...
               <footer style={{ padding: '1rem 1.5rem', borderTop: '1px solid var(--med-gray)', display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'var(--white)', position: 'relative' }}>
                  <div style={{ position: 'relative' }}>
                     <button onClick={() => setShowAttachments(!showAttachments)} style={{ background: 'var(--light-gray)', border: 'none', color: 'var(--text-main)', cursor: 'pointer', width: '40px', height: '40px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s', transform: showAttachments ? 'rotate(45deg)' : 'none' }}>
                        <Plus size={20} />
                     </button>
                     {showAttachments && (
                        <div style={{ position: 'absolute', bottom: 'calc(100% + 0.5rem)', left: 0, background: 'var(--white)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-lg)', padding: '0.5rem', display: 'flex', flexDirection: 'column', gap: '0.5rem', border: '1px solid var(--med-gray)', zIndex: 10 }}>
                           <button style={{ background: 'transparent', border: 'none', color: 'var(--text-main)', cursor: 'pointer', padding: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem', whiteSpace: 'nowrap', textAlign: 'left', borderRadius: 'var(--radius-md)' }}><Paperclip size={18} /> Document</button>
                           <button style={{ background: 'transparent', border: 'none', color: 'var(--text-main)', cursor: 'pointer', padding: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem', whiteSpace: 'nowrap', textAlign: 'left', borderRadius: 'var(--radius-md)' }}><ImageIcon size={18} /> Gallery</button>
                        </div>
                     )}
                  </div>
                  <div style={{ flex: 1 }}>
                     <input type="text" placeholder="Type a message..." style={{ width: '100%', padding: '0.75rem 1rem', borderRadius: '2rem', border: '1px solid var(--med-gray)', outline: 'none', fontFamily: 'inherit', fontSize: '0.9rem' }} />
                  </div>
